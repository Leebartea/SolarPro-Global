...
- **The Open Graph share card did not exist.** All nine pages have carried
  `<meta property="og:image" content=".../og-cover.jpg">` since v1.0 and the
  file had never been in the repository, so every share of the demo — the
  thing a marketplace reviewer and a buyer both see first — rendered as a
  bare link. `assets/images/og-cover.jpg` is now generated by
  `scripts/make-og-cover.js` from the template's own brand tokens and its
  bundled OFL fonts: type, the brand orange and a drawn panel grid, no
  photograph, so it is first-party artwork with no redistribution question
  attached. It is regenerable rather than a committed one-off, so a brand
  change does not strand it.
- **The tag was also relative, which would have failed silently even once the
  file existed.** Facebook, LinkedIn and X fetch the card with a plain HTTP
  client that does not resolve a relative path against the page. All nine
  pages now carry the absolute URL against the `www.example.com` placeholder
  origin that `SITE_URL` already rewrites at build time, so the live demo
  stamps its real domain automatically and the buyer's search-and-replace is
  the same one the canonical tags need.
- **`local-assets` now reads `<meta content="…">`, not just `src` and `href`.**
  That is why this survived thirteen green checks and a W3C run: the sweep
  looked at two attributes and the defect lived in a third. The check fails on
  a missing card image and on a relative one, and both failure modes were
  mutation-tested before this release. Generalises: a check scoped to the
  attributes that break *visibly* cannot see a defect that only breaks in
  someone else's timeline.
- **`markup-validity` had never opened the buyer documentation.** It iterated
  `PAGES`, which is the nine site pages; the help file is in `ALL_HTML`. So
  `docs/documentation.html` carried an `h1` → `h3` skip — error class 4, the
  exact class this check exists to pin — through every green run since the
  check was written, and the Nu validator confirmed it on the first request.
  The check now iterates `ALL_HTML`, the two overview sub-headings are `h2`,
  and all ten HTML files return zero W3C errors. Third instance of the same
  shape after `PAGES` and `INCLUDE`: a check scoped to a hand-maintained
  list passes over the work it never reads.
- **Documentation corrected.** The customisation guide said "all 6 pages
  already reference this path" — a count two releases stale — and told the
  buyer to create the file rather than replace it. It now states the real
  count, the placeholder, and the absolute-URL requirement with the tag to
  copy.
