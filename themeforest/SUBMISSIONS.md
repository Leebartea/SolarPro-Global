...
| — | 1.6.1 | *not submitted* | — | Superseded by 1.6.2. The `og:image` referenced a file that had never existed, on all nine pages — every share of the demo rendered as a bare link. |
| — | 1.6.2 | **ready to submit** | — | 14 checks green. W3C: 9 pages, 0 errors. Lighthouse desktop: a11y and best-practices 100 on all nine, perf 95–100, SEO 100 on eight (404 is 63 by design — it is `noindex`). Envato 3:2 cover built. OG share card now ships and is absolute-URL'd; `local-assets` reads `<meta content>` and was mutation-tested against both failure modes. AI policy re-read 7 Aug 2026, unchanged. |
